...
const CATALOG_SMART_TAB_KEY = "stock_distribuidora_catalog_smart_tab_v1";
...
const ui = {
  ...
  /** Pestaña activa en Catálogo inteligente: variants | combos | promos */
  catalogSmartTab: "variants",
  /** El panel de contenido (título + formularios) solo se muestra tras elegir un botón. */
  catalogSmartContentOpen: false,
};
...
const dom = {
  ...
  catalogSmartTablist: $("catalogSmartTablist"),
  catalogSmartPlaceholder: $("catalogSmartPlaceholder"),
  catalogSmartPanesWrap: $("catalogSmartPanesWrap"),
  catalogSmartCloseBtn: $("catalogSmartCloseBtn"),
  supportTelegramLink: $("supportTelegramLink"),
  supportTelegramHint: $("supportTelegramHint"),
  supportTelegramChip: $("supportTelegramChip"),
...
function resolveCatalogSmartTab(raw) {
  const v = String(raw || "").trim();
  if (v === "combos" || v === "promos" || v === "variants") return v;
  return "variants";
}

function applyCatalogSmartTab() {
  const tab = resolveCatalogSmartTab(ui.catalogSmartTab);
  ui.catalogSmartTab = tab;
  const root = dom.catalogSmartTablist;
  if (root) {
    root.querySelectorAll("[data-catalog-tab]").forEach((btn) => {
      const id = btn.getAttribute("data-catalog-tab");
      const active = ui.catalogSmartContentOpen && id === tab;
      btn.classList.toggle("is-active", active);
      btn.setAttribute("aria-selected", active ? "true" : "false");
    });
  }
  const showPanes = ui.catalogSmartContentOpen;
  if (dom.catalogSmartPlaceholder) {
    dom.catalogSmartPlaceholder.classList.toggle("hidden", showPanes);
  }
  if (dom.catalogSmartPanesWrap) {
    dom.catalogSmartPanesWrap.classList.toggle("hidden", !showPanes);
  }
  document.querySelectorAll(".catalog-smart-pane[data-catalog-pane]").forEach((pane) => {
    if (!showPanes) {
      pane.classList.add("hidden");
      return;
    }
    const active = pane.getAttribute("data-catalog-pane") === tab;
    pane.classList.toggle("hidden", !active);
  });
}

function resolveSupportTelegramHref(raw) {
  const s = String(raw == null ? "" : raw).trim();
  if (!s) return "";
  if (/^https?:\/\//i.test(s)) return s;
  if (/^\+[A-Za-z0-9_-]+$/.test(s)) return `https://t.me/${s}`;
  const u = s.replace(/^@/, "");
  if (/^[A-Za-z][A-Za-z0-9_]{3,31}$/.test(u)) return `https://t.me/${encodeURIComponent(u)}`;
  const digits = s.replace(/\D/g, "");
  if (digits.length >= 10 && digits.length <= 15 && /^\d+$/.test(digits)) {
    if (digits.startsWith("54") && digits.length >= 12) return `https://t.me/+${digits}`;
    if (digits.length === 10) return `https://t.me/+549${digits}`;
    return `https://t.me/+${digits}`;
  }
  return "";
}

function supportTelegramDisplayLabel() {
  const raw =
    typeof window !== "undefined" && window.SUPPORT_TELEGRAM_LABEL != null
      ? String(window.SUPPORT_TELEGRAM_LABEL).trim()
      : "";
  return raw || "Soporte técnico";
}

function initSupportTelegramFab() {
  const link = dom.supportTelegramLink;
  const hint = dom.supportTelegramHint;
  const chip = dom.supportTelegramChip;
  const dock = $("supportTelegramDock");
  if (!link) return;
  const label = supportTelegramDisplayLabel();
  const href = resolveSupportTelegramHref(typeof window !== "undefined" ? window.SUPPORT_TELEGRAM_USER : "");
  if (dock) dock.setAttribute("aria-label", `${label} · Telegram`);
  if (href) {
    link.href = href;
    link.setAttribute("target", "_blank");
    link.setAttribute("rel", "noopener noreferrer");
    link.removeAttribute("data-needs-config");
    link.setAttribute("title", `${label} · Telegram`);
    link.setAttribute("aria-label", `Abrir Telegram · ${label}`);
    if (hint) hint.textContent = label;
    if (chip) chip.textContent = label.length > 13 ? "Soporte" : label;
  } else {
    link.href = "#";
    link.removeAttribute("target");
    link.removeAttribute("rel");
    link.dataset.needsConfig = "1";
    link.setAttribute("title", "Configurá Telegram en firebase-config.js");
    link.setAttribute("aria-label", "Soporte Telegram no configurado");
    if (hint) hint.textContent = "Configurá Telegram";
    if (chip) chip.textContent = "Ayuda";
  }
}
...
dom.catalogSmartTablist?.addEventListener("click", (e) => {
  const btn = e.target.closest("[data-catalog-tab]");
  if (!btn || !dom.catalogSmartTablist.contains(btn)) return;
  const next = btn.getAttribute("data-catalog-tab");
  if (!next) return;
  if (ui.catalogSmartContentOpen && next === ui.catalogSmartTab) return;
  ui.catalogSmartContentOpen = true;
  ui.catalogSmartTab = resolveCatalogSmartTab(next);
  try {
    localStorage.setItem(CATALOG_SMART_TAB_KEY, ui.catalogSmartTab);
  } catch (_) {}
  applyCatalogSmartTab();
});

dom.catalogSmartCloseBtn?.addEventListener("click", () => {
  if (!ui.catalogSmartContentOpen) return;
  ui.catalogSmartContentOpen = false;
  applyCatalogSmartTab();
  try {
    dom.catalogSmartTablist?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  } catch (_) {}
});

dom.supportTelegramLink?.addEventListener("click", (e) => {
  if (dom.supportTelegramLink?.dataset.needsConfig === "1") {
    e.preventDefault();
    toast(
      "Configurá window.SUPPORT_TELEGRAM_USER en firebase-config.js: usuario, URL t.me/…, invitación +código (grupo → Invitar) o celular AR solo dígitos."
    );
  }
});
...
function init() {
  ...
  ui.catalogSmartContentOpen = false;
  try {
    ui.catalogSmartTab = resolveCatalogSmartTab(localStorage.getItem(CATALOG_SMART_TAB_KEY));
  } catch (_) {
    ui.catalogSmartTab = "variants";
  }
  applyCatalogSmartTab();
  renderAll();
  initSupportTelegramFab();
  setupPwa();
  updateSyncStatus();
  flushSync().catch(() => {
    if (!cloud.enabled) {
